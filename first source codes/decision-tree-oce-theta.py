...
Theta=[0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95]
##################################################
...
def test_example(example, node, class_index):
    if (node.is_leaf == True):
        ...
        if (example[node.attr_split_index] >= node.attr_split_value):
            ...
            mple(example, node.lower_child, class_index)
...
def main():
    ...
    if (len(args) < 2):
        ...
        if ("-v" in args):
            ...
            print ("Initial (pre-pruning) validation set score: " + str(100*max(best_scores)) +"%, the theta that corresponds to this value is"+str(Theta[best_scores.index(max(best_scores))]))
        if ("-p" in args):
            if("-v" not in args):
                print ("Error: You must validate if you want to prune")
            else:
                root=Roots[best_scores.index(max(best_scores))]
