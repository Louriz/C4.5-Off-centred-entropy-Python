...
def read_data(dataset, datafile, datatypes):
    ...
    dataset.attr_types=['true','true','true','true','false']
...
def compute_tree(dataset, parent_node, classifier,datatypes):
        ...
        if (dataset.attributes[attr_index] != classifier):
            ...
            if(datatypes[attr_index]=='true'):
                ...
                for val in attr_value_list:
                    ...
                    local_gain = calc_gain(dataset, classifier, dataset_entropy, attr_index,datatypes, val) # calculate the gain if we split on this value
      
                    if (local_gain >= local_max_gain):
                        local_max_gain = local_gain
                        local_split_val = val
            else:
            	local_max_gain=calc_gain(dataset, classifier, dataset_entropy, attr_index,datatypes)
    ...
    for i in range(len(split_val)):
    	...
    	node.children=[compute_tree(children_datasets[j],node,classifier,datatypes) for j in range(len(split_val))]
    	#print(node.children)
    return node
...
##################################################
def calc_gain(dataset,classifier, entropy, attr_index,datatypes, val=None):
    ...
    if(datatypes[attr_index]=='false'):
        ...
        for i in range(len(unique_li)):
            ...
            attr_entropy+=calc_dataset_entropy(datasets[i],classifier)*len(datasets[i].examples)/total_examples
...
def main():
    ...
    if (len(args) < 2):
        ...
        #datatypes=['true','true','false','true','true','true','true','false'] #  ecoli
        #datatypes=['true','true','true','true','false'] #  iris
